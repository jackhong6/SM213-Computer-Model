...
public class MainMemory extends AbstractMainMemory {
  ...
  @Override public int bytesToInteger (byte byteAtAddrPlus0, byte byteAtAddrPlus1, byte byteAtAddrPlus2, byte byteAtAddrPlus3) {
    int b0 = ((int) byteAtAddrPlus0) << 24;
    b0 = 0xff000000 & b0;

    int b1 = ((int) byteAtAddrPlus1) << 16;
    b1 = 0x00ff0000 & b1;

    int b2 = ((int) byteAtAddrPlus2) << 8;
    b2 = 0x0000ff00 & b2;

    int b3 = ((int) byteAtAddrPlus3);
    b3 = 0x000000ff & b3;
  ...
  @Override public byte[] integerToBytes (int i) {
    byte[] myMem = new byte[4];
    myMem[0] = (byte) ((i >> 24) & 0xff);
    myMem[1] = (byte) ((i >> 16) & 0xff);
    myMem[2] = (byte) ((i >> 8) & 0xff);
    myMem[3] = (byte) (i & 0xff);
    return myMem;
  }
  ...
  @Override protected byte[] get (int address, int length) throws InvalidAddressException {
    ...
    if (address < 0 || address + length - 1 >= mem.length) throw new InvalidAddressException();
  ...
  @Override protected void set (int address, byte[] value) throws InvalidAddressException {
    if (address < 0 || address + value.length - 1 >= mem.length) throw new InvalidAddressException();
